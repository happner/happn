	...
	*/
	it('should initialize the clients', function(callback) {
		...
		try{
		  ...
		  publisherclient = new happn.client({config:{host:'localhost', port:testport, secret:test_secret, parser:'binary'}}, function(e){

		    if (e)
		      return callback(e);

		    listenerclient = new happn.client({config:{host:'localhost', port:testport, secret:test_secret, parser:'binary'}}, function(e){
		        callback(e);
		    });
