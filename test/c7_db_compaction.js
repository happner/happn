describe('c7_db_compaction', function() {
  ...
  var test_file4 = __dirname + '/test-resources/c7/test/' + test_id + '4.test';
  var test_file4a = __dirname + '/test-resources/c7/test/' + test_id + '4a.test';
  ...
  it('starts a db with 2 files, does some random activity, compacts the db, checks that both files have been compacted', function(callback){
    getService(serviceConfig4, function(e, serviceInstance){
      ...
      getClient(clientConfig4, function(e, client){
        ...
        randomActivity4 = new RandomActivityGenerator(client4, {pathPrefix:['/c7_db_compaction/' + test_id + '/4/','/c7_db_compaction/' + test_id + '/4a/']});
