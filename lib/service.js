...
happn = require('happn');
server = null;
happn.service.create({},function(e,s) {
...
module.exports = {
  /*stop:function(options, done){
    log.$$DEBUG('stopping a');
    log.$$TRACE('stopping trace a', options, stopCB);
          ...
          async.eachSeries(Object.keys(happn.services), 
            ...
            function(e) {
              ...
              for (var key in _this.connections)
                _this.connections[key].destroy();

              log.$$TRACE('killed connections', _this.connections);
