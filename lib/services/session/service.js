...
SessionService.prototype.disconnectAllClients = function(timeout, callback){
  ...
  try{
    ...
    _this.each(function(sessionData, sessionDataCallback){
      ...
      if (client) _this.disconnect(client, {reason:'server-side-disconnect', reconnect:false}, sessionDataCallback);
...
SessionService.prototype.__finalizeDisconnect = function(client, sessionData, options, callback){
  ...
  _this.__session_info.remove(client.sessionId, function(e) {
    ...
    if (e)
      return _this.happn.services.error.handleFatal('failed removing session data from cache for disconnect', e, 'session');

    delete _this.__sessions[client.sessionId];
    _this.emit('disconnect', sessionData);

    _this.happn.services.queue.pushSystem({action:'disconnect', options:{reconnect:false}, session:sessionData})
...
SessionService.prototype.onDisconnect = function (client, options) {

  if (!options) options = {};

  if (!client.__serverDisconnected) this.clientDisconnect(client, options);
};
