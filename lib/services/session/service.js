...
SessionService.prototype.disconnectSession = function(sessionId, message){
  ...
  this.disconnect(client, message);
};
...
SessionService.prototype.stop = function (options, callback) {
  try {
    ...
    if (this.primus) {
      ...
      if (!options.client_disconnect_timeout) options.client_disconnect_timeout = 5000;
      ...
      _this.disconnectAllClients(options.client_disconnect_timeout, function(e){
...
SessionService.prototype.disconnectAllClients = function(timeout, callback){
  var _this = this;
  ...
  var timeoutRef = setTimeout(function(){
    callback(new Error('client disconnection timed out'));
  }, timeout);
  ...
  try{
    ...
    _this.each(function(sessionData, sessionDataCallback){
      ...
      if (client) _this.disconnect(client, 'server-side-disconnect', sessionDataCallback);
...
SessionService.prototype.handleMessage = Promise.promisify(function (message, client) {
  ...
  _this.__session_info.get(client.sessionId, function(e, sessionData){
    ...
    _this.happn.services.queue.pushInbound({raw:message, session:sessionData})
...
SessionService.prototype.__disconnectMessage = function(client, reason){
  return {
    raw: {
      action:'disconnect',
      data:reason,
    ...
    session:{
      id:client.sessionId,
      protocol: this.happn.services.protocol.current()
    }
  };
};
...
SessionService.prototype.clientDisconnect = function (client, callback) {
  ...
  _this.__session_info.get(client.sessionId, function(e, sessionData) {
    ...
    _this.__finalizeDisconnect(client, sessionData, callback);

  });
};

SessionService.prototype.__finalizeDisconnect = function(client, sessionData, callback){
  ...
  _this.__session_info.remove(client.sessionId, function(e) {
    ...
    _this.emit('disconnect', sessionData);
...
SessionService.prototype.disconnect = function (client, reason, callback) {
  ...
  _this.__session_info.get(client.sessionId, function(e, sessionData) {
    ...
    client.end({_meta:{type:'system'}, eventKey:'server-side-disconnect', data:reason}, {reconnect:false});
