...
CacheService.prototype.new = function(name, opts){
  ...
  if (opts.type == 'LRU') this.__caches[name] = new LRUCache(opts.cache);
  ...
  try{
    ...
    if (cache){
      ...
      if (cache == this.config.defaultCacheName){
        this.__defaultCache.clear();
      }else{
        if (this.__caches[cache]) delete this.__caches[cache];
...
CacheService.prototype.update = function(itemKey, data, callback){
  this.__defaultCache.update(itemKey, data, callback);
};

CacheService.prototype.increment = function(itemKey, by, callback){
  this.__defaultCache.increment(itemKey, by, callback);
};

CacheService.prototype.set = function(itemKey, data, opts, callback){
  this.__defaultCache.set(itemKey, data, opts, callback);
};

CacheService.prototype.get = function(itemKey, opts, callback){
  this.__defaultCache.get(itemKey, opts, callback);
};
