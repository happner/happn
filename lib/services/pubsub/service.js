...
PubSubService.prototype.initialize = function(config, done) {
	...
	try{
		...
		if (config['security-mode'] == 'unsecure'){
			...
			_this.login = function(message, socket){
				_this.handleDataResponseSocket(null, message, {data: _this.attachSession(socketInstance, {}, message.data.info)}, socket);
			}
...
PubSubService.prototype.login = function(message, socketInstance){
	...
	if (!message.data)
		return _this.handleDataResponseSocket(new Error('unauthenticated request, no credentials'), message, null, socketInstance);
...
PubSubService.prototype.authorizeRequest = function(socket, message, callback){
	...
	if (_this.trusted[socket.session.id] == undefined) return _this.handleDataResponseSocket(_this.securityService.AccessDeniedError(), message, null, socket);
	...
	_this.securityService.authorize(socket.session, message.path.replace(/^\/(?:REMOVE|SET|ALL)@/, ''), message.action, function(e, authorized){
		...
		if (!authorized) return _this.handleDataResponseSocket(_this.securityService.AccessDeniedError(), message, null, socket);
...
PubSubService.prototype.publish = function(message, payload) {
	...
	for (var allPath in _this.__listeners_wildcard_ALL){
	  	...
	  	if (_this.happn.utils.wildcardMatch(allPath, messageChannel))
