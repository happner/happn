...
module.exports = {
	...
	initialize: function(config, done) {
		...
		//console.log('INIT:', config, _this.happn.services);

		if (config.enableProxy)
			_this.handle_message = _this.proxy_message;
		else
			_this.handle_message = _this.process_message;

		_this.utils = _this.happn.utils;

		_this.primus = new Primus(_this.happn.server, config.primus);

		_this.primus.before('proxy', _this.onBefore.bind(_this));
	...
	proxy_message:function(message, socketInstance){
		var _this = this;

		//console.log('proxying..?', _this.happn.services);

		if (!_this.happn.services.proxy.relayWebSocket(message, socketInstance))
			_this.process_message(message, socketInstance);
	},
	process_message: function(message, socketInstance) {
		var _this = this;
	...
	onBefore: function(req, res){

		console.log('onBefore happened');
		console.log(req.url);
	...
	onConnect: function(socket) {
		...
		//console.log('connect on address:::', socket.address);

		socket.on('error', function(err) {
			_this.utils.log('socket error', 'error', 'service-pubsub', err);
		});
	...
	emitToAudience: function(audienceGroup, publication, channel) {
		...
		if (audienceGroup[channel]) {
			...
			for (var sessionIndex in audienceGroup[channel]) {
				...
				////console.log('in emit', sessionIndex);
	...
	publish: function(message, payload) {
        ...
        _this.utils.matchPaths(messageChannel, _this.__listeners_wildcard_ALL).map(function(allPath){
        	_this.emitToAudience(_this.__listeners_wildcard_ALL[allPath], publication, allPath);
        });

        _this.utils.matchPaths(messageChannel, _this['__listeners_wildcard_' + action]).map(function(actionPath){
        	_this.emitToAudience(_this['__listeners_wildcard_' + action], publication, actionPath);
        });
