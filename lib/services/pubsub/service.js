...
module.exports = {
	...
	handle_message: function(message, socketInstance) {
		...
		try {
				if (message.action == 'login') {
					...
					if (!message.data)
						return _this.handleDataResponse('Unauthenticated request, no credentials', message, response, socketInstance);

					_this.attachSession(socketInstance, message.data);
					return _this.handleDataResponse(null, message, socketInstance.session, socketInstance);
					...
					if (_this.validateRequest(socketInstance)){
						...
						if (message.action == 'get'){
							...
							_this.addListener(message.path, socketInstance.session.index);
	...
	onDisconnect: function(socket) {
		var _this = this;
		_this.disconnect(socket.session.index);
	},
	...
	addListener: function(channel, sessionIndex) {
		var _this = this;
		...
		if (!audienceGroup[channel][sessionIndex]) 
			audienceGroup[channel][sessionIndex] = 1;
		else 
			audienceGroup[channel][sessionIndex] += 1;

	},
	decrementEventReference: function(audienceGroup, sessionIndex, channel, decrementBy){
		...
		if (!channel){
			...
			if (audienceGroup[channel] && audienceGroup[channel][sessionIndex]){
				audienceGroup[channel][sessionIndex] -= decrementBy;//decrement the listener counter
	...
	removeListener: function(sessionIndex, channel, decrementBy) {
		var _this = this;
		if (channel == "*"){ 
			...
			_this.decrementEventReference(_this.getAudienceGroup(channel), sessionIndex, channel, decrementBy);
		}
	},
	...
	validateRequest:function(socket){
		...
		if (_this.__trusted[socket.session.index] == socket.session.token)
			return true;
	...
	attachSession:function(socket, credentials){
		...
		_this.__trusted[sessionIndex] = sessionToken;

		socket.session = {token:sessionToken, index:sessionIndex};
	...
	detachSession:function(socket){
		...
		if (_this.__sessions[socket.session.index]){
			...
			delete _this.__trusted[socket.session.index];
