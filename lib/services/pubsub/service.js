...
PubSubService.prototype.createResponse = function(e, message, response, local) {
	...
	if (response._event) {
		console.log('TODO: make first creator')
	}
	...
	if (!local) {
		payload = {
			...
			_store: response._store,
		}
		delete response._store;
	...
	if (e) {
		payload._event.status = 'error';
		// responseData.payload = e.toString();
		payload._event.error = e.toString();
		return payload;
	}
	...
	// if (!local && Array.isArray(response)) {

	// 	console.log('IS ARRAY');

	// 	// encode response as array WITH _event property
	// 	// ie. 
	// 	//   response = [thing,thing,thing]
	// 	//   response._event = {...}
	// 	// 
	// 	// carry array across as __ARRAY
	// 	// restored on other side

	// 	return {
	// 		__ARRAY: response,
	// 		_event: response._event
	// 	}
	// }
...
PubSubService.prototype.handleDataResponseSocket = function(e, message, response, socket) {
	console.log('handleDataResponseSocket');
...
PubSubService.prototype.handleDataResponseLocal = function(e, message, response, handler, client) {
	...
	if (handler) {
		if (response._event.status == 'error') return handler(response);
		return handler(null, response);
	} else {
		if (response._event.status == 'error') {
