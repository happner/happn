...
PubSubService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    if (!config.primus) config.primus = {};

    _this.primus = new Primus(_this.happn.server, config.primus);
    _this.primus.on('connection', _this.onConnect.bind(_this));
    _this.primus.on('disconnection', _this.onDisconnect.bind(_this));

    //we add the transform methods

    if (_this.happn.config.encryptPayloads)  _this.primus.transform('incoming', _this.incomingDecrypt.bind(_this));
    if (_this.happn.config.compressPayloads)  _this.primus.transform('incoming', _this.transformUncompress.bind(_this));
    if (config.incomingSpy)  _this.primus.transform('incoming', config.incomingSpy.bind(config.incomingSpy));

    _this.primus.transform('outgoing', _this.transformPayload.bind(_this));
    if (config.outgoingSpy)  _this.primus.transform('outgoing', config.outgoingSpy.bind(config.outgoingSpy));
    if (_this.happn.config.compressPayloads)  _this.primus.transform('outgoing', _this.transformCompress.bind(_this));
    if (_this.happn.config.encryptPayloads)  _this.primus.transform('outgoing', _this.outgoingEncrypt.bind(_this));
...
PubSubService.prototype.transformPayload = function(packet, next){

  if (packet.data && packet.data._meta && !packet.data._meta.tag){
    if (packet.data._meta.action == 'set') delete packet.data.data;
    delete packet.data._meta._id;
    delete packet.data._meta.sessionId;
  }

  next();
};

PubSubService.prototype.transformCompress = function(packet, next){
  //console.log('transform compress:::', packet);
  next();
};

PubSubService.prototype.transformUncompress = function(packet, next){
  //console.log('transform uncompress:::', packet);
  next();
};

PubSubService.prototype.incomingDecrypt = function (packet, next) {
  ...
  if (this.happn.config.encryptPayloads) {
    ...
    try {
      ...
      if (packet.data.action == 'login') {
        ...
        packet.data.data = JSON.parse(this.happn.services.crypto.asymmetricDecrypt(publicKey, this.happn.services.security._keyPair.privateKey, packet.data.data.encrypted).toString());
...
PubSubService.prototype.outgoingEncrypt = function (packet, next) {
...
PubSubService.prototype.__doSubscriptionCallback = function (socketInstance, message) {
  ...
  if (message.options.initialCallback || message.options.initialEmit) {
    ...
    _this.dataService.get(dataPath, {sort: {'modified': 1}}, function (e, initialItems) {
      ...
      if (message.options.initialCallback) {
...
PubSubService.prototype.removeListener = function (sessionIndex, channel, parameters) {
  if (channel == '*') {
    this.decrementEventReference(this.__listeners_SET, sessionIndex);
    this.decrementEventReference(this.__listeners_REMOVE, sessionIndex);
    this.decrementEventReference(this.__listeners_ALL, sessionIndex);
    this.decrementEventReference(this.__listeners_ONALL, sessionIndex);
    this.decrementEventReference(this.__listeners_wildcard_ALL, sessionIndex);
    this.decrementEventReference(this.__listeners_wildcard_SET, sessionIndex);
    this.decrementEventReference(this.__listeners_wildcard_REMOVE, sessionIndex);
  } else {
    this.decrementEventReference(this.getAudienceGroup(channel), sessionIndex, channel, parameters.refCount);
  }
};
  ...
  if (audienceGroup[channel] != null && audienceGroup[channel] != undefined) {
    ...
    for (var sessionIndex in audienceGroup[channel]) {
      ...
      if (this.__sessions[sessionIndex].intraProc) {
        ...
        if (!opts.serialized) opts.serialized = JSON.stringify(publication)
