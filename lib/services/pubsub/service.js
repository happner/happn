	...
	try{
		...
		if (config.secure){
			...
			_this.login = function(message, socket){
				_this.handleDataResponseSocket(null, message, {data: _this.attachSession(socket, {}, message.data.info)}, socket);
...
PubSubService.prototype.login = function(message, socketInstance){
	...
	if (!message.data)
		return _this.handleDataResponseSocket(_this.securityService.AccessDeniedError('no credentials'), message, null, socketInstance);
	...
	_this.securityService.login(message.data, function(e, session){
		...
		// _this.emit('authentic', {info: socket.session.info});
		// _this.emit('authentic', {info: message.data.info});
