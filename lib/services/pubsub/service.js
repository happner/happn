...
PubSubService.prototype.incomingTransform = function(packet, next){
	...
	try{
		...
		if (packet.data.parameters && packet.data.parameters.encryptPayload){

			console.log('encrypted packet in:::', packet);

			var session = this.getSession(packet.data.sessionId);
			if (!session) throw new Error('unable to find session with id: ' + packet.data.sessionId);
...
PubSubService.prototype.outgoingTransform = function(packet, next){
	console.log('packet out:::', packet.data);
	next();
}
