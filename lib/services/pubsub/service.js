  ...
  if (message.request.options.initialCallback || message.request.options.initialEmit) {
    ...
    _this.dataService.get(dataPath, {sort: {'modified': 1}}, function (e, initialItems) {
      ...
      if (message.request.options.initialCallback) {
        ...
        doCallback();
        _this.__emitInitialValues(message.request.eventId, channel, message.sessionId, initialItems, message.protocol, function(e){
...
PubSubService.prototype.__emitPublication = function (publication, channel, opts, sessionId, callback){
  ...
  if (opts.clone) emitPublication = this.happn.services.utils.clone(publication);
  ...
  var message = {
    raw:{
      ...
      opts:opts,
      action:'emit',
      protocol:publication.protocol?publication.protocol:opts.protocol?opts.protocol:'happn'
    },
    sessionId:sessionId
  };

  this.happn.services.queue.pushOutbound(message, callback);
