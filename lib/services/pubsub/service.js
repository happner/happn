...
PubSubService.prototype.stop = function(options, done){
    ...
    try{
    	...
    	if (_this.primus){
    		_this.primus.destroy({close:false}, done);
    	}
    	else
        	done();
    }catch(e){
	...
	try{
		...
		if (this.happn.config.encryptPayloads){
			...
			if (packet.data.action == 'login'){
				//TODO - check if we have encrypted the credentials with the server public key
				console.log('login packet in:::', packet);
				...
				console.log('encrypted packet in:::', packet);
				var session = this.getSession(packet.data.sessionId);
				packet.data = JSON.parse(this.happn.services.crypto.symmetricDecrypt(session.secret, session.secretSalt, packet.data.encrypted));
			}

			console.log('packet transformed:::', packet);
...
PubSubService.prototype.outgoingTransform = function(packet, next){
	console.log('packet out:::', packet, this.happn.config);
	...
	if (this.happn.config.encryptPayloads){
		...
		if (packet.data._meta.action == 'login'){
			...
			packet.data.sessionId = packet.data._meta.sessionId;
			packet.data.encrypted = this.happn.services.crypto.symmetricEncrypt(session.secret, session.secretSalt, JSON.stringify(packet.data));
			delete packet.data.data;
			delete packet.data._meta;
		}

		console.log('outgoing packet transformed:::', packet);
	}
...
PubSubService.prototype.createResponse = function(e, message, response, local) {
	...
	console.log('doing createResponse:::', message);
	...
	console.log('createResponse:::', _meta);
...
PubSubService.prototype.handle_message = function(message, socketInstance) {
	...
	try {
		if (message.action == 'login') {
			...
			console.log('have description:::', serviceDescription);
...
PubSubService.prototype.validateRequest = function(socket, message, callback){
	...
	console.log('validating request:::', message);
...
PubSubService.prototype.attachSession = function(socket, session, info){
	...
	socket.session = _this.happn.utils.clone(session);//so we can preserve the session secret

	_this.__sessions[sessionIndex] = socket;
	_this.trusted[socket.session.id] = sessionIndex;

	delete session['secret'];//we delete this - as we are passing back the ECIES encrypted secret
	delete socket.session['encryptedSecret'];//we dont need this on the server side
