...
PubSubService.prototype.handle_message = function(message, socketInstance) {
	...
	try {
			if (message.action == 'login') {
				...
				_this.attachSession(socketInstance, message.data, message.data.info, function(e, session) {

					if (e) {
						return _this.handleDataResponseSocket(e, message, {}, socketInstance);
					}

					 _this.handleDataResponseSocket(null, message, {data: socketInstance.session}, socketInstance);
				});
				...
				if (_this.validateRequest(socketInstance, message)){
...
PubSubService.prototype.validateRequest = function(socket, message){
	var _this = this;
...
PubSubService.prototype.attachSession = function(socket, credentials, info, callback){
	var _this = this;
	var sessionToken;

	// socket.session = _this.securityService.newSession(credentials);

	_this.securityService.newSession(credentials, null, function(e, session) {
...
PubSubService.prototype.connect = function(handler, callback) {
	var _this = this;
	...
	_this.attachSession(socket, undefined, info, callback);
}
