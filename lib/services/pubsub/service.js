...
PubSubService.prototype.createResponse = function(e, message, response) {
	...
	response._.type = 'response';
	response._.status = 'ok';
	response._.published = false;
	response._.eventId = message.eventId;

	if (e) {
		response._.status = 'error';
		// responseData.payload = e.toString();
		response._.error = e.toString();
	} else {
...
PubSubService.prototype.handleDataResponseLocal = function(e, message, response, handler, client) {
	...
	if (handler) {
		if (response._.status == 'error') return handler(response);
		return handler(null, response);
	} else {
		if (response._.status == 'error') {
