...
PubSubService.prototype.handle_message = function(message, socketInstance) {
	...
	try {
		if (message.action == 'login') {
			...
			_this.validateRequest(socketInstance, message, function(e){
...
PubSubService.prototype.validateRequest = function(socket, message, callback){
	...
	if (message.action == 'remove') return callback();//you dont need permission to stop listening

	_this.securityService.authorize(socket.session, message.path, message.action, callback);
}
...
PubSubService.prototype.__notifyClient = function(socket, eventKey, data){
	//TODO -writes to a special client channel
	console.log('notifying client:::', eventKey, data);
}
		...
		try{
			...
			_this.__sessions.every(function(socket){
				...
				if (whatHappnd == 'delete-user'){
					...
					if (socket.session.user.username == changedData.obj.username){
