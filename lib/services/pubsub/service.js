...
PubSubService.prototype.processMessage = Promise.promisify(function(message, callback){
  try{
    ...
    if (message.request.action === 'on') {
      ...
      if (!message.request.options.noPublish) return _this.publish(message.request, message.response, function(e){
        callback(e, message);
      });
    }
...
PubSubService.prototype.emitToAudience = function (publication, channel, opts, callback) {
  ...
  if (channel.indexOf('/2_websockets_embedded_sanity/') > -1)
    console.log('CONTACT:::');
  ...
  if (audienceGroup && audienceGroup[channel] != null) {

    async.each(Object.keys(audienceGroup[channel]), function(sessionId, emitCallback){

      var decoupledPublication;

      if (opts.deepCopy) {
        //we deep clone for intra process comms
        decoupledPublication = _this.happn.services.utils.clone(publication, true);
      } else if (opts.noCopy){
        decoupledPublication = publication;
      }
      else {
      ...
      decoupledPublication._meta.channel = channel;
      decoupledPublication._meta.sessionId = sessionId;
      ...
      var message = {
        request:{
          publication:decoupledPublication,
          channel:channel,
          opts:opts,
          action:'emit',
      ...
      _this.happn.services.queue.pushOutbound(message, emitCallback);
...
PubSubService.prototype.publish = function (request, payload, publishCallback) {
  ...
  async.waterfall([
    ...
    function(payload, callback){
      _this.emitToAudience(payload, messageChannel, opts, function(e){
    ...
    function(payload, callback){
      ...
      async.each(_this.__listeners_wildcard_ALL, function(allPath, wildcardAllCallback){
    ...
    function(payload, callback){
      ...
      async.each(wildcardActionGroup, function(actionPath, wildcardAllCallback){
    ...
    if (messageChannel.indexOf('/2_websockets_embedded_sanity/') > -1)
      console.log('CONTACT:::');
