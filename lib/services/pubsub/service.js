...
PubSubService.prototype.createResponse = function(e, message, response, local) {
	...
	response = response || {__NULL:{}}

	if (response) {

		response._meta = response._meta || {};

		response._meta.type = 'response';
		response._meta.status = 'ok';
		response._meta.published = false;
		response._meta.eventId = message.eventId;
...
PubSubService.prototype.publish = function(message, payload) {
	...
	var type = payload._meta.type;
	// var publication = payload;
	// payload._meta.timestamp = Date.now();
  ...
  channels.every(function(channel){
  	_this.emitToAudience(_this.getAudienceGroup(channel), payload, channel);
  	return true;
  });
  ...
  // delete payload._meta.action;
  // delete payload._meta.timestamp;
