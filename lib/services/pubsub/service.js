...
PubSubService.prototype.getAudienceGroup = function(channel) {
  ...
  // optimisation: indexOf(*) below is:
  // 1. repeated twice, that is wasteful
  // 2. probably faster as `channel[channel.length - 1] == '*'`
...
PubSubService.prototype.emitToAudience = function(publication, channel) {
	...
	if (audienceGroup[channel] != null && audienceGroup[channel] != undefined) {
        ...
        //the packet data is decoupled here, even if the data goes over the wire
          ...
          // optimisation: the above serialization could be performed as many as 5 times for
          // the same publish (because that's how many times emitToAudience() is called with the
          // same payload) this can be avoided
          // - the same does not apply to the clone that is necessary for every audience member
...
PubSubService.prototype.publish = function(message, payload) {
	...
	for (var allPath in this.__listeners_wildcard_ALL){
                                                // optimisation: this replace below can be removed by storing
                                                // the channel name with the *@ in __listeners_wildcard_ALL
                                                // from the get go.
                                                // (will mean altering slightly how the subscribe works and
                                                // the channel name in the _meta will be different in messages)
