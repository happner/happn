...
PubSubService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    if (!config.primusOpts) config.primusOpts = {};
    ...
    if (message.action == 'login') {
      ...
      _this.validateRequest(socketInstance, message, function () {
        ...
        _this.authorizeRequest(socketInstance, message, function () {
          ...
          if (message.action == 'get') {
          ...
          } else if (message.action == 'revoke-session') {

            _this.securityService.revokeSession(socketInstance.session, 'CLIENT', function(e){
            ...
            _this.removeListener(socketInstance.session.index, message.path, message.options);
...
PubSubService.prototype.authorizeRequest = function (socket, message, callback) {
  ...
  if (message.action == 'revoke-session') return callback();//you dont need permission to revoke your own session
...
PubSubService.prototype.validateRequest = function (socket, message, callback) {
  ...
  if (['on', 'get', 'set', 'off', 'remove','revoke-session'].indexOf(message.action) == -1)
...
PubSubService.prototype.attachSession = function (socket, session, info) {
  ...
  this.emit('authentic', {info: info, session: session});
  ...
  if (session.cookieName !== 'happn_token') loginResponse.cookieName = session.cookieName;
  if (session.cookieDomain) loginResponse.cookieDomain = session.cookieDomain;
...
PubSubService.prototype.detachSession = function (socket) {
  ...
  this.emit('disconnect', {info: socket.session.info, session: socket.session});
...
PubSubService.prototype.getAudienceSessions = function(sessions, opts){
  ...
  return sessionIds.reduce(function(matchingSessionIds, sessionId){
    ...
    var client = _this.__sessions[sessionId];

    if (opts.targetClients.indexOf(client.session.id) > -1) matchingSessionIds.push(sessionId);
...
PubSubService.prototype.emitToAudience = function (publication, channel, opts) {
  ...
  if (audienceGroup[channel] != null && audienceGroup[channel] != undefined) {
    ...
    var sessions = _this.getAudienceSessions(audienceGroup[channel], opts);

    sessions.forEach(function(sessionIndex){

      socket = _this.__sessions[sessionIndex];
...
PubSubService.prototype.publish = function (message, payload) {
  ...
  var opts = {            // 1. to allow shared .serialized between repetitive calls to emitToAudience()
    ...
    targetClients: message.options.targetClients?message.options.targetClients:null
  };
