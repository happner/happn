...
PubSubService.prototype.createResponse = function(e, message, response, local) {
	...
	if (!local && response instanceof Array) {

		// Can't transmit array with properties (_event), nest the array.
	...
	// response._store = response._store || {};
	// response._store.path = response.path;
	// response._store.id = response._id;
	...
	// console.log('local', local, 'payload', payload);


	// response._event = response._event || {}; // TODO: is first creator of _event? (then remove ||)
	// response._event.type = 'response';
	// response._event.status = 'ok';
	// response._event.published = false;
	// response._event.id = message.eventId;

	if (e) {
		response._event.status = 'error';
		// responseData.payload = e.toString();
		response._event.error = e.toString();
		return response;
	}

	if (response && response.snapshot) {
		// turning off snapshot (tag) publish until clear on purpose and behaviour
		return response;
	}
...
PubSubService.prototype.handleDataResponseLocal = function(e, message, response, handler, client) {
	var response = this.createResponse(e, message, response, true);
	var decoded;
	...
	if (!response.__NULL) {
		if (response.paths) {
			...
			decoded = response;
			decoded._store.path = response.path;
			decoded._store.id = response._id;
			delete response.path;
			delete response._id;
		}
		else if (Array.isArray(response)) {
			decoded = this.decodeArrayResponse(response);
		} else {
			decoded = response.data;
			Object.defineProperty(decoded, '_store', {
				value: response._store,
				enumerable: false,
				configurable: true
			});
			Object.defineProperty(decoded, '_event', {
	...
	if (handler) {
		if (decoded) {
			if (decoded._event.status == 'error') {
				...
				if (_this.validateRequest(socketInstance)){
					...
					if (message.action == 'get'){
						...
						if (message.parameters.noStore) return _this.handleDataResponseSocket(
							null, message, _this.dataService.formatSetData(message
...
PubSubService.prototype.emitToAudience = function(audienceGroup, publication, channel) {
	var _this = this;
	if (audienceGroup[channel]) {
		publication._event.channel = channel.toString();
...
PubSubService.prototype.publish = function(message, payload) {
	...
	publication._store.path = publication._store.path || payload.path;
