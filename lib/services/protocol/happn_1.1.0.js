...
ProtocolHappn.prototype.__validateIn = Promise.promisify(function(message, callback){
  ...
  try{
    ...
    if (['on', 'get', 'set', 'off', 'remove', 'describe', 'emit', 'login', 'disconnect'].indexOf(message.raw.action) == -1) throw _this.happn.services.error.ValidationError('Unknown request action: ' + message.raw['action']);
...
ProtocolHappn.prototype.transformIn = Promise.promisify(function(message, callback){
  ...
  try{
    ...
    if (message.raw.action  == 'login' && message.raw.data && message.raw.data.encrypted) {
      ...
      message.request.data.encrypted = true;//session is set to encrypted;
      message.session.encrypted = true;//for this call down as well
...
ProtocolHappn.prototype.transformOut = Promise.promisify(function(message, callback){
  ...
  try{
      ...
      //if (message.session.encrypted) message.request.publication = {encrypted:_this.happn.services.crypto.symmetricEncryptObject(message.request.publication, message.session.secret)};
...
ProtocolHappn.prototype.emit = Promise.promisify(function(message, session, callback){
  ...
  try{
    ...
    if (session.encrypted) message.request.publication = {encrypted:this.__encryptMessage(message.request.publication, session.secret)};
...
ProtocolHappn.prototype.fail = function(e, message){
  ...
  return new Promise(function(resolve, reject) {
    ...
    try{
      ...
      if (message.session.encrypted) message.response = _this.__encryptMessage(message.response, message.session.secret);
