...
ProtocolHappn.prototype.emit = function(message, callback){
  ...
  if (client.intraProc) {
    ...
    decoupledPublication = this.happn.services.utils.clone(message.request.publication, true);
  } else {
    ...
    if (!opts.serialized) opts.serialized = JSON.stringify(message.request.publication);
  ...
  decoupledPublication._meta.channel = message.request.channel.toString();
  decoupledPublication._meta.sessionId = message.sessionId;
