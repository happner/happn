...
QueueService.prototype.initialize = Promise.promisify(function (config, callback) {
  ...
  this.__queue = async.queue(this.happn.services.protocol.process, this.config.concurrency);
  this.__sequentialQueue = async.queue(this.happn.services.protocol.process, 1);

  this.__queue.drain = this.drain;
  this.__sequentialQueue.drain = this.sequentialDrain;
