...
DataEmbeddedService.prototype.initialize = function (config, callback) {
  ...
  try {
    ...
    if (config.datastores && config.datastores.length > 0) {
      ...
      for (var configIndex in  _this.config.datastores) {
        ...
        if (!datastoreConfig.name)
          return callback(new Error('invalid configuration, datastore config in position ' + configIndex + ' has no name'));
    ...
    if (_this.config.compactInterval) {
      return _this.startCompacting(_this.config.compactInterval, callback);
    }
