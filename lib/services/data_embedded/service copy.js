...
DataEmbeddedService.prototype.getOneByPath = function(path, fields, callback){
     ...
     _this.db.findOne({'_meta.path':path}, fields, function(e, findresult){
...
DataEmbeddedService.prototype.get = function(path, parameters, callback){
    var _this = this;
     ...
     try{
        ...
        if (path.indexOf('*') >= 1) {
            single = false;
            dbCriteria.$and.push({"_meta.path":{ $regex: new RegExp('^' + path.replace('*',''))}});
        }
        else {
            dbCriteria.$and.push({"_meta.path":path});
        }
        ...
        cursor.exec(function(e, items){
            ...
            if (parameters.options.path_only) {
                items = items.map(function(itm) {return itm._meta});
            ...
            if (single) {
                ...
                items[0]._meta.id = items[0]._id;
                delete items[0]._id;
                return callback(e, items[0]);
            }

            items = items.map(function(item) {
                item._meta.id = item._id;
                delete item._id;
                return item;
            });
...
DataEmbeddedService.prototype.upsertInternal =function(path, setData, options, dataWasMerged, callback){
    var _this = this;
    var setParameters = {$set: {"data":setData.data, "_meta.modified":setData._meta.modified, "_meta.path":setData._meta.path}};

    _this.db.update({"_meta.path":path}, setParameters, {upsert:true}, function(err, response, created) {
...
DataEmbeddedService.prototype.remove = function(path, options, callback){
    ...
    var criteria = {"_meta.path":path};

    if (path.indexOf('*') > -1) {
        criteria = {"_meta.path":{ $regex: new RegExp('^' + path.replace('*', '')) }};
    }
