...
DataEmbeddedService.prototype.initialize = function(config, done){
    ...
    try{
        ...
        if (config.datastores && config.datastores.length > 0){
            ...
            console.log('_this.datastores:::',_this.datastores);
...
DataEmbeddedService.prototype.startCompacting = function(interval, dataStoreKey, callback){
    try{
        interval = parseInt(interval.toString());
        if (interval < 5000)
            throw new Exception('interval must be at least 5000 milliseconds');

        _this.__iterateDataStores(dataStoreKey, function(key, dataStore, next){
            _this.__compacting[key] = dataStore.db;
            _this.__compacting[key].setAutocompactionInterval(interval);
            next();
        },
        callback);
...
DataEmbeddedService.prototype.stopCompacting = function(dataStoreKey, callback){
    ...
    _this.__iterateDataStores(dataStoreKey, function(key, dataStore, next){
        dataStore.db.persistence.stopAutocompaction();
...
DataEmbeddedService.prototype.__compact = function(db, cb){
    db.on('compaction.done', cb);
    db.persistence.compactDatafile();
}

DataEmbeddedService.prototype.__iterateDataStores = function(dataStoreKey, operator, done){
    ...
    try{
        ...
        if (dataStoreKey){
            ...
            if (!_this.datastores)
                return done(new Error('datastore with key ' + dataStoreKey + ', specified, but multiple datastores not configured'));

            if (!_this.datastores[dataStoreKey])
                return done(new Error('datastore with key ' + dataStoreKey + ', does not exist'));

            return operator(dataStoreKey, _this.datastores[dataStoreKey], done);
        }
        ...
        if (_this.datastores){
            ...
            return operator("default",_this.dbInstance, done);
        }
...
DataEmbeddedService.prototype.compact = function(dataStoreKey, callback){
    ...
    try{
        ...
        _this.__iterateDataStores(dataStoreKey, function(key, dataStore, next){
            _this.__compact(dataStore.db, next);
        },
        callback);
