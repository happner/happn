...
DataEmbeddedService.prototype.initialize = function(config, done){
    ...
    try{
        ...
        if (_this.config.compactInterval)
            return _this.startCompacting(_this.config.compactInterval, done);
...
DataEmbeddedService.prototype.startCompacting = function(dataStoreKey, interval, callback){
    try{
        ...
        _this.__iterateDataStores(dataStoreKey, function(key, dataStore, next){
            ...
            _this.__compacting[key].persistence.setAutocompactionInterval(interval);
            next();
        },
        callback);
