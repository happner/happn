...
DataEmbeddedService.prototype.initialize = function(config, done){
    ...
    try{
        ...
        _this.db.ensureIndex({unique:true, fieldName:'__path'}, done);
...
DataEmbeddedService.prototype.getOneByPath = function(path, fields, callback){
     ...
     _this.db.findOne({__path:path}, fields, function(e, findresult){
    ...
    var _this = this;
     ...
     var insertTag = function(snapshotData){
        ...
        var tagData = {
            ...
            __path:path + '/tags/' + shortid.generate()
        }
...
DataEmbeddedService.prototype.get = function(path, parameters, callback){
    var _this = this;
     ...
     try{
        ...
        console.log('TODO: child_id, wildcard');

        if (parameters.options.child_id){

            dbCriteria.$and.push({"__path":path});
            dbCriteria.$and.push({"data._id":parameters.options.child_id});
           
            dbFields = {_id: 0, 'data.$': 1};

        }else if (path.indexOf('*') >= 1) {
            dbCriteria.$and.push({"__path":{ $regex: new RegExp('^' + path.replace('*',''))}});
        }else {
            dbCriteria.$and.push({"__path":path});
        }
        ...
        console.log('dbCriteria', dbCriteria);
...
DataEmbeddedService.prototype.formatSetData = function(path, data, timestamp){
    ...
    data._store.modified = timestamp;
    data._store.path = path;
...
DataEmbeddedService.prototype.transformSetData =function(path, data, timestamp){
    ...
    if (setData instanceof Array) {
        setData.map(function(item, index, array){
            ...
            // put _store onto each element, each with unique id
            // for singular reference-ability using options.child_id

            if (!item._store || item._store.id == null) {
                item._store = item._store || {};
                item._store.modified = timestamp
                item._store.id = shortid.generate();
                item._store.path = setData._store.path;
...
DataEmbeddedService.prototype.upsert = function(path, data, options, callback){
    ...
    if (options.set_type == 'child'){
        ...
        return _this.db.update({__path:path}, {$push: {data:posted}}, {upsert:true}, function(err, updatedCount) {
...
DataEmbeddedService.prototype.upsertInternal =function(path, setData, options, dataWasMerged, callback){
    ...
    _this.db.update({"__path":path}, setParameters, {upsert:true}, function(err, response) {
...
DataEmbeddedService.prototype.remove = function(path, options, callback){
    ...
    if (options && options.child_id){
         ...
         _this.db.update({__path:path}, { $pull: { "data": {'_id':options.child_id}}}, function(err, updated){
            callback(err, {data:options.child_id, removed:updated});
         });     

    }else{

        var criteria = {"__path":path};

        if (path.indexOf('*') > -1)
            criteria = {"__path":{ $regex: path }};
