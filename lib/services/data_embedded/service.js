...
DataEmbeddedService.prototype.initialize = function(config, done){
    ...
    try{
        ...
        _this.db.ensureIndex({unique:true, fieldName:'_store.path'}, done);
...
DataEmbeddedService.prototype.getOneByPath = function(path, fields, callback){
     ...
     _this.db.findOne({'_store.path':path}, fields, function(e, findresult){
...
DataEmbeddedService.prototype.get = function(path, parameters, callback){
    var _this = this;
     ...
     try{
        ...
        if (path.indexOf('*') >= 1) {
            single = false;
            dbCriteria.$and.push({"_store.path":{ $regex: new RegExp('^' + path.replace('*',''))}});
        }
        else {
            dbCriteria.$and.push({"_store.path":path});
        }
        ...
        cursor.exec(function(e, items){
            ...
            if (parameters.options.path_only) {
                items = items.map(function(itm) {return itm._store});
...
DataEmbeddedService.prototype.upsertInternal =function(path, setData, options, dataWasMerged, callback){
    ...
    _this.db.update({"_store.path":path}, setParameters, {upsert:true}, function(err, response) {
...
DataEmbeddedService.prototype.remove = function(path, options, callback){
    ...
    var criteria = {"_store.path":path};

    if (path.indexOf('*') > -1) {
        criteria = {"_store.path":{ $regex: new RegExp('^' + path.replace('*', '')) }};
    }
