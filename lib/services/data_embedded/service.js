    ...
    try{
        ...
        if (config.datastores && config.datastores.length > 0){
            ...
            for (var configIndex in  _this.config.datastores){
                ...
                if (datastoreConfig.settings && datastoreConfig.settings.dbfile)
                    datastoreConfig.settings.filename = datastoreConfig.settings.dbfile;

                if (datastoreConfig.settings && datastoreConfig.settings.filename)
            ...
            _this.db = function(path){
                ...
                for (var datastoreConfigName in _this.datastoreConfig){
                    if (_this.datastores[datastoreConfigName].config.patterns){
