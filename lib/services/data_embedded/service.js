...
DataEmbeddedService.prototype.__compact = function(db, interval, cb){
    if (interval != null && interval < 5000) return cb(new Error('compaction interval too low'));

    if (!key)
        key = "dbInstance";

    if (!interval){
        db.on('compaction.done', cb)
        db.persistence.compactDatafile();
    }else{
        db.persistence.setAutocompactionInterval(interval);
        _this.__compacting.push(db);
        cb();
    }

}

//compacting job,running in background - switches periodic compaction on and off
var __compacting = {};
DataEmbeddedService.prototype.compactOff = function(key){
    __compacting.map(function(db){
        return db.persistence.stopAutocompaction();
    });
}

DataEmbeddedService.prototype.compact = function(interval, dataStoreKey, callback){

    var _this = this;
    try{

        if (dataStoreKey && _this.config.datastores[dataStoreKey]){
            if (__compacting)
        }

        if (interval)
            _this.compactOff();//stop all interval compaction

        var toCompact = [];

        if (dataStoreKey && _this.config.datastores[dataStoreKey])
            toCompact.push(_this.config.datastores[dataStoreKey].db);
        else if (_this.config.datastores && _this.config.datastores.length > 0)
            Object.keys(_this.datastores).map(function(key){
                 toCompact.push(_this.config.datastores[key].db);
            });
        else
            toCompact.push(_this.dbInstance);


        async.eachSeries(toCompact, function(db, next){
            this.__compact(db, interval, next);
        }, callback);
    ...
    var _this = this;
     ...
     var insertTag = function(snapshotData){
        ...
        var tagData = {
            ...
            _tag:tag,
            _id: '/_TAGS' + path + '/' + uuid.v4().replace(/-/g,'')
        }
