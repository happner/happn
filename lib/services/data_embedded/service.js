...
DataEmbeddedService.prototype.parseCriteria = function(criteria){
    ...
    traverse(criteria).forEach(function (value) {
        ...
        if (value){
            ...
            if (value.bsonid) {
                this.update(value.bsonid);
                return;
            }

            if (this.key == '_meta.created'){
               criteria['createdAt'] = value;
               this.remove();
               return;
            }

            if (this.key == '_meta.modified'){
               criteria['updatedAt'] = value;
               this.remove();
               return;
            }
    ...
    var _this = this;
     ...
     try{
        ...
        if (parameters.criteria){
            single = false;
            dbCriteria.$and.push(_this.parseCriteria(parameters.criteria));
        }

        console.log('searching:::', dbCriteria, dbFields);
...
DataEmbeddedService.prototype.upsert = function(path, data, options, callback){
    ...
    if (options.merge){
        ...
        return _this.getOneByPath(path, null, function(e, previous){
            ...
            setData.createdAt = previous.createdAt;
...
DataEmbeddedService.prototype.transform = function(dataObj, additionalMeta){
    ...
    transformed._meta = {
        path:dataObj._id,
        // created:moment.utc(dataObj.createdAt).valueOf(),
        // modified:moment.utc(dataObj.updatedAt).valueOf(),
        tag:dataObj._tag
    }

    if (dataObj.createdAt)
        transformed._meta.created = dataObj.createdAt.toJSON();

    if (dataObj.updatedAt)
        transformed._meta.modified = dataObj.updatedAt.toJSON();
