...
DataEmbeddedService.prototype.compact = function(callback){

    var _this = this;
    try{
        var compactDb = function(db, cb){
            db.on('compaction.done', cb)
            db.persistence.compactDatafile();
        }

        if (_this.config.datastores && _this.config.datastores.length > 0)
            async.eachSeries(Object.keys(_this.datastores), function(dataStoreKey, next){
                compactDb(_this.datastores[dataStoreKey].db, next);
            }, callback);
        else
            compactDb(_this.dbInstance, callback)
    }catch(e){
        callback(e);
    }
}
