...
SystemService.prototype.processMessage = Promise.promisify(function(message, callback){

  if (message.transformed.action === 'describe') message.response = {data:this.getDescription()};
  return callback();
});
...
SystemService.prototype.processMessage = function(message, callback){
  try{
    ...
    if (message.transformed.action === 'describe') message.response = {data:_this.getDescription()};
    console.log('SYS HAPPENENING:::');
