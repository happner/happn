...
SecurityUsers.prototype.securityDirectoryChanged = function (whatHappnd, changedData) {
  ...
  if (['link-group', 'unlink-group', 'delete-user', 'delete-group'].indexOf(whatHappnd) > -1) {
    ...
    _this.sessionService.each(function (session, sessionCallback) {

      try{

        if (whatHappnd == 'link-group') {

          if (changedData._meta.path.indexOf('/_SYSTEM/_SECURITY/_USER/' + session.user.username + '/_USER_GROUP/') == 0) {

            var groupName = changedData._meta.path.replace('/_SYSTEM/_SECURITY/_USER/' + session.user.username + '/_USER_GROUP/', '');

            session.user.groups[groupName] = changedData;
            session.permissionSetKey = _this.securityService.generatePermissionSetKey(session.user);
          }
        }

        if (whatHappnd == 'unlink-group') {

          if (changedData.indexOf('/_SYSTEM/_SECURITY/_USER/' + session.user.username + '/_USER_GROUP/') == 0) {

            var groupName = changedData.replace('/_SYSTEM/_SECURITY/_USER/' + session.user.username + '/_USER_GROUP/', '');
            delete session.user.groups[groupName];
        ...
        if (whatHappnd == 'delete-user') {
          ...
          if (session.user.username == userName) {
            //TODO: unimplemented
            _this.sessionService.disconnectSession(session.id, 'security directory update: user deleted');
          }
        }

        if (whatHappnd == 'delete-group') {

          if (session.user.groups[changedData.obj.name]) {
            delete session.user.groups[changedData.obj.name];
            session.permissionSetKey = _this.securityService.generatePermissionSetKey(session.user);
          }
        }
      ...
      if (e) return _this.happn.services.error.handleFatal('failure updating session security data', e, 'security users');
    });
  }
