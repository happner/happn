...
CheckPoint.prototype.__constructPermissionSet = function (session, callback) {
  ...
  if (session.isToken){ //we are dealing with a decoded token
    ...
    _this.securityService.getUser(session.username, function(e, user){
      ...
      if (user == null) return callback(_this.securityService.AccessDeniedError('user ' + session.username + ' has been deleted or does not exist'));
...
CheckPoint.prototype.initialize = function (config, securityService, callback) {
  ...
  try {
    ...
    _this.cacheService = securityService.cacheService;
...
CheckPoint.prototype._authorizeSession = function (session, path, action, callback) {
  ...
  try {
    ...
    _this.__checkInactivity(session, policy, function(e, ok) {
      ...
      if (e) return callback(e);
      if (!ok) callback(null, false, 'session inactivity threshold reached');

      _this.__checkUsageLimit(session, policy, function(e, ok){

        if (e) return callback(e);
        if (!ok) callback(null, false, 'session usage limit reached');
        ...
        if (action == 'login') return callback(null, true, null, true);//passthrough

        if (policy.permissions){

          passthrough = _this.__checkSessionPermissions(policy, path, action);
