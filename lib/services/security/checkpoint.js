...
CheckPoint.prototype._authorizeSession = function (session, path, action, callback) {
  ...
  try {
    ...
    if (policy.ttl > 0 && (Date.now()) > session.timestamp + policy.ttl) return callback(new Error('expired session token'));

    _this.__checkInactivity(session, policy, function(e, ok) {

      if (e) return callback(e);
      if (!ok) return callback(new Error('session inactivity threshold reached'));

      _this.__checkUsageLimit(session, policy, function(e, ok){

        if (e) return callback(e);
        if (!ok) return callback(new Error('session usage limit reached'));
