...
SecurityService.prototype.processMessage = Promise.promisify(function(message, callback){
  try{
    ...
    if (_this.config.secure){
      ...
      if (message.request.action == 'login') return _this.login(message.request.data, function(e, session){

        if (e) return callback(e);

        _this.happn.services.session.updateSession(session);
    ...
    } else if (message.raw.action == 'emit'){

      // coming back from pubsub, this sushi is fresh

    } else if (message.request.action == 'login') message.response = this.generateEmptySession();
...
SecurityService.prototype.__initializeCheckPoint = function(config, callback){
  ...
  this.__checkpoint = new checkpoint({logger: this.log});
  this.__checkpoint.initialize(config, this, callback);
...
SecurityService.prototype.login = function (credentials, callback) {
  ...
  if (credentials.username && (credentials.password || credentials.digest)) {
    ...
    return _this.users.getUser(credentials.username, function (e, user) {
      ...
      if (e) return callback(e);
      if (user == null) return callback(_this.happn.services.error.AccessDeniedError('Invalid credentials'));
      ...
      if (credentials.digest){
        ...
        return _this.verifyAuthenticationDigest(credentials, function(e, valid){
          ...
          if (!valid) return callback(_this.happn.services.error.AccessDeniedError('Invalid credentials'));
      ...
      _this.users.getPasswordHash(credentials.username, function (e, hash) {
        ...
        _this.matchPassword(credentials.password, hash, function (e, match) {
          ...
          if (!match) return callback(_this.happn.services.error.AccessDeniedError('Invalid credentials'));
  ...
  else callback(_this.services.happn.error.AccessDeniedError('Invalid credentials'));
...
SecurityService.prototype.authorize = function (sessionId, path, action, callback) {

  var _this = this;

  var completeCall = function(err, authorized, reason){
    callback(err, authorized, reason);
    if (_this.config.logSessionActivity) _this.__logSessionActivity(sessionId, path, action, err, authorized, reason, function(e){
      if (e) _this.log.warn('unable to log session activity: ' + e.toString());
    });
  };

  _this.__checkRevocations(sessionId, function(e, authorized, reason){
    ...
    if (!session) return completeCall(null, false, 'session with id: ' + sessionId + ' does not exist');
