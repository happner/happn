...
SecurityService.prototype.validate = function(validationType, options, obj, callback){
    ...
    if (validationType == 'group'){
        ...
        if (options.parent){
            if (!options.parent.systemPath)
                return callback(new Error('validation error: parent group path is not in your request, have you included the systemPath?'));
            //path, parameters, callback
            return _this.dataService.get(options.parent.systemPath, {}, function(e, result){

                if (e) return callback(e);

                if (!result)
                    return callback(new Error('validation error: parent group does not exist: ' + options.parent.systemPath));
    ...
    if (validationType == 'permission'){
        ...
        if (!permissionGroup.systemPath)
            return callback(new Error('validation error: permission group path is not in your request, have you included the systemPath?'));
        //path, parameters, callback
        return _this.dataService.get(permissionGroup.systemPath, {}, function(e, result){

            if (e) return callback(e);

            if (!result)
                return callback(new Error('validation error: permission group does not exist: ' + permissionGroup.systemPath));
...
SecurityService.prototype.serialize = function(objectType, obj, options){
    ...
    returnObj.systemPath = obj._meta.path;

    if (objectType == 'group'){
       if (options.excludeRelated && returnObj.systemPath.indexOf('_PERMISSION'))//the case of this being called by serializeAll, and we have permissions in the array
            return null;
    }
    ...
    if (objectType == 'user-group'){
        if (options.excludeRelated && returnObj.systemPath.indexOf('_USER_GROUP'))//the case of this being called by serializeAll, and we have user groups in the array
            return null;
    }
    ...
    try{
        _this.validate('group', options, group, function(e){
            ...
            if (options.parent)//we have checked in validate
                groupPath = options.parent.systemPath + '/' + group.name;
            else
...
SecurityService.prototype.addPermission = function(path, permission, group, callback){
    ...
    _this.validate('permission', [permission,group], path, function(e){
        ...
        var permissionPath = group.systemPath + '/_PERMISSION' + path;
