...
SecurityService.prototype.revokeSession = function(session, reason, callback){
  ...
  if (!session.ttl || session.ttl == Infinity) this.log.warn('revoking a session without a ttl means it stays in the revocation list forever');

  this.__cache_revoked_sessions.set(session.id, {reason:reason}, {ttl:session.ttl}, callback);
};
...
SecurityService.prototype.authorize = function (session, path, action, callback) {
  ...
  var completeCall = function(err, authorized){
    ...
    if (_this.__sessionManagementActive) _this.__logSessionActivity(session, path, action, function(e){
