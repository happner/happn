    ...
    if (credentials.username && credentials.password) {
        ...
        _this.dataService.get(userPath, {}, function(e, result) {
            ...
            _this.verifyHash(credentials.password, actualHashed, function(e, match) {
                ...
                if (!match) return callback(new Error('Invalid'));
...
SecurityService.prototype.linkGroup = function(group, user, options, callback){
    ...
    try{
        _this.validate('user-group', options, [group, user], function(e){
            if (e) return callback(e);

            var groupLinkPath = '/_SYSTEM/_SECURITY/_USER/' + user.name + '/_USER_GROUP/' + group.name;

            _this.dataService.upsert(groupLinkPath, {}, {merge:true}, function(e, result){
                if (e) return callback(e);
