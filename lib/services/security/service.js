...
SecurityService.prototype.initialize = function (config, callback) {
  try {
    ...
    else config.defaultNonceTTL = this.happn.utils.toMilliseconds(config.defaultNonceTTL);
    ...
    else config.sessionActivityTTL = this.happn.utils.toMilliseconds(config.sessionActivityTTL);
...
SecurityService.prototype.__logSessionActivity = function(session, path, action, err, authorized, reason, callback){
...
SecurityService.prototype.generateSession = function (user, callback, info) {
...
SecurityService.prototype.__initializeProfiles = function(config, callback){
  ...
  config.profiles.forEach(function(profile){
    if (profile.policy.ttl && profile.policy.ttl != Infinity) profile.policy.ttl = _this.happn.utils.toMilliseconds(profile.policy.ttl);
    if (profile.policy.inactivity_threshold && profile.policy.ttl != Infinity) profile.policy.inactivity_threshold = _this.happn.utils.toMilliseconds(profile.policy.inactivity_threshold);
  });
...
SecurityService.prototype.__loginOK = function(credentials, user, callback){
  ...
  return this.generateSession(user, callback, credentials.info);
};

SecurityService.prototype.__checkLockedOut = function(username){

  var _this = this;

  if (!_this.config.accountLockout || !_this.config.accountLockout.enabled) return false;

  if (!username) return false;

  var existingLock = _this.__locks.get(username);

  return (existingLock != null && existingLock.attempts >= _this.config.accountLockout.attempts);
};

SecurityService.prototype.__loginFailed = function(username, callback){

  var _this = this;

  if (username && (_this.config.accountLockout && _this.config.accountLockout.enabled)) {

    var currentLock = _this.__locks.get(username);

    if (!currentLock) currentLock = {attempts:0};

    currentLock.attempts++;

    _this.__locks.set(username, currentLock, {ttl:_this.config.accountLockout.retryInterval});
  }

  return callback(_this.AccessDeniedError('Invalid credentials'));
};
...
SecurityService.prototype.login = function (credentials, callback) {
  ...
  if (_this.__checkLockedOut(username)) return callback(_this.AccessDeniedError('Account locked out'));
  ...
  if (credentials.token) {
    ...
    try{
      ...
      return _this.authorize(previousSession, null, 'login', function(e, authorized, reason){
        ...
        if (e) return _this.__loginFailed(username, callback);

        if (!authorized) return _this.__loginFailed(username, callback);

        _this.getUser(username, function (e, user) {

          if (user == null) return _this.__loginFailed(username, callback);
      ...
      return _this.__loginFailed(username, callback);
    }
  }
  ...
  if (credentials.username && (credentials.password || credentials.digest || credentials.token)) {
    ...
    return _this.getUser(credentials.username, function (e, user) {
      ...
      if (user == null) return _this.__loginFailed(credentials.username, callback);

      if (credentials.digest){

        if (user.publicKey != credentials.publicKey) return _this.__loginFailed(credentials.username, callback);

        return _this.verifyAuthenticationDigest(credentials, function(e, valid){

          if (e) return callback(e);

          if (!valid) return _this.__loginFailed(credentials.username, callback);
      ...
      _this.getPasswordHash(credentials.username, function (e, hash) {
        ...
        _this.matchPassword(credentials.password, hash, function (e, match) {
          ...
          if (!match) return _this.__loginFailed(credentials.username, callback);
...
SecurityService.prototype.authorize = function (session, path, action, callback) {
  ...
  var completeCall = function(err, authorized, reason){
    ...
    if (_this.config.logSessionActivity) _this.__logSessionActivity(session, path, action, err, authorized, reason, function(e){
      if (e) _this.log.warn('unable to log session activity: ' + e.toString());
    });
  };

  _this.__checkRevocations(session, function(e, authorized, reason){

    if (e) return callback(e);

    if (!authorized) return completeCall(null, false, reason);

    _this.__checkpoint._authorizeSession(session, path, action, function(e, authorized, reason, passthrough){//check the session ttl, expiry,permissions etc.
