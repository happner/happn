...
function SecurityService(opts) {
  ...
  this.__authenticationNonceCache = {};
...
SecurityService.prototype.initialize = function (config, callback) {
  try {
    ...
    _this.__loadTokenRevokeList(config, function (e) {
...
SecurityService.__checkRevocations = function(sessionId, callback){
  if (this.revoked_sessions.get(sessionId) != null) return callback(new Error('session with id ' + sessionId + ' has been revoked'));
  callback();
};

SecurityService.prototype.revokeSession = function(sessionId, reason, callback){
  ...
  this.revoked_sessions.set(sessionId, {reason:reason}, callback);

};

SecurityService.prototype.restoreSession = function(callback){
...
SecurityService.prototype.__logSessionActivity = function(session, callback){
...
SecurityService.prototype.revokeToken = function(token, callback){

  var _this = this;

  _this.dataService.upsert('/_SYSTEM/_SECURITY/_TOKEN/REVOKE', token, {}, function(e){
...
SecurityService.prototype.authorize = function (session, path, action, callback) {
  ...
  _this.__checkRevocations(session, function(e){
    ...
    _this.__checkpoint._authorizeSession(session, path, action, function(e, passthrough){//check the session ttl, expiry,permissions etc.
      ...
      if (passthrough) return callback(null, true);
