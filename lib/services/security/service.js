...
SecurityService.prototype.generateSession = function (user, callback) {
  ...
  this.__profileSession(session);//TODO: session ttl and activity threshold is set here
...
SecurityService.prototype.__initializeProfiles = function(profileConfig){

  var _this = this;

  _this.__cache_Profiles = {
    ind:{},
    items:[]
  };
  ...
  config.profiles.push({//default stateless profile

  });

  config.profiles.push({//default stateful

  });

  config.profiles.map(function(profile, ind){

    _this.__cache_Profiles[profile.name] = ind;//allow for a quick lookup
    _this.__cache_Profiles.push(profile);//actual profile
...
SecurityService.prototype.listGroups = function (groupName, options, callback) {
  ...
  try {
    this.dataService.get(searchPath, {sort: "_id"}, function (e, results) {
      if (e) return callback(e);
...
SecurityService.prototype.getGroup = function (groupName, options, callback) {
  ...
  try {
    ...
    this.dataService.get(searchPath, {sort: "_id"}, function (e, result) {
...
SecurityService.prototype.listUsers = function (userName, options, callback) {
  ...
  try {
    //list all user items without USER_GROUP in their paths
    this.dataService.get(searchPath, {criteria: {$not: {"_id": {$regex: new RegExp(".*_USER_GROUP.*")}}}}, function (e, results) {

      if (e) return callback(e);

      callback(null, this.serializeAll('user', results, options));
    }.bind(this));
