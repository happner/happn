...
SecurityService.prototype.initialize = function(config, done){
    try{
        ...
        if (!this.happn.config || !this.happn.config.secure) return done();
        ...
        if (!config.sessionTokenSecret)
           config.sessionTokenSecret = uuid.v4();
        ...
        var startedInit = Date.now();

        _this._ensureKeyPair(config, function(e){

            console.log('startuptimes:::ensure-keypair:', Date.now() - startedInit);

            if (e) return done(e);

            startedInit = Date.now();

            _this._ensureAdminUser(config, function(e){

                if (e) return done(e);

                console.log('startuptimes:::ensure-admin:', Date.now() - startedInit);
                ...
                _this.__checkpoint.initialize(config, _this, function(e){
                    ...
                    console.log('startuptimes:::checkpoint:', Date.now() - startedInit);
...
SecurityService.prototype.dataChanged = function(whatHappnd, changedData, additionalInfo){
    ...
    _this.updateCaches(whatHappnd, changedData, additionalInfo);
    ...
    if (validationType == 'user'){
        ...
        if (!obj.password){
            return _this.dataService.get('/_SYSTEM/_SECURITY/_USER/' + obj.username, {}, function(e, result){
                ...
                checkOverwrite('/_SYSTEM/_SECURITY/_USER/' + obj.username, obj.username);
