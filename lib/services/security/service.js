...
var __groupCache = LRU(__groupCacheOpts);
var __userCache = LRU(__userCacheOpts);
var __passwordCache = LRU(__userCacheOpts);//alongside the user cache
...
SecurityService.prototype._ensureAdminUser = function(config, callback){
    ...
    _this.dataService.get('/_SYSTEM/_SECURITY/_USER/_ADMIN*', {}, function(e, response){
        ...
        _this.__upsertGroup(config.adminGroup, {}, function(e, response){

            if (e) return callback(e);

            _this.__upsertUser(config.adminUser, {}, function(e, response){
    ...
    try{
        ...
        _this._ensureKeyPair(config, function(e){
            ...
            _this._ensureAdminUser(config, function(e){
                ...
                _this.__checkpoint = new checkpoint();
                _this.__checkpoint.initialize(config, _this, done);
...
SecurityService.prototype.dataChanged = function(whatHappnd, changedData){
    __dataHooks.every(function(hook){
        if (changedData){
            var decoupled = JSON.parse(JSON.stringify(changedData));
            return hook.apply(hook, [whatHappnd, decoupled]);
        }else{
             return hook.apply(hook, [whatHappnd, changedData]);
        }
    });
}
...
SecurityService.prototype.login = function(credentials, callback) {
    ...
    if (credentials.username && credentials.password) {
        ...
        return _this.getUser(credentials.username, {includePassword:true}, function(e, user){
            ...
            if (e) return callback(e);
            if (user == null) return callback(_this.AccessDeniedError('user does not exist'));

            _this.verifyHash(credentials.password, __passwordCache.get(user.username), function(e, match) {

                if (e) return callback(e);

                if (!match) return callback(_this.AccessDeniedError('login failed'));
        ...
        callback(_this.AccessDeniedError());
...
SecurityService.prototype.serialize = function(objectType, obj, options){
    ...
    if (options.deepCopy)
        returnObj = JSON.parse(JSON.stringify(obj.data));
    else
        returnObj = obj.data;
    ...
    if (objectType == 'user'){ 
       if (!options.includePassword) {
        console.log('removing pwd', options.includePassword);
    ...
    _this.__prepareUser(user, options, function(e, prepared){
        ...
        _this.dataService.upsert(userPath, prepared, {merge:true}, function(e, result){
            ...
            if (prepared.password)
                __passwordCache.del(user.username);
            
            var upserted = _this.serialize('user', result, options);

            __userCache.del(user.username);
...
SecurityService.prototype.__upsertGroup = function(group, options, callback){
    ...
    console.log('group upsert in happn:::', group);
    ...
    _this.dataService.upsert(groupPath, group, {merge:true}, function(e, result){
        ...
        var upserted = _this.serialize('group', result, options);
          ...
          console.log('group upserted in happn:::', upserted);

         __groupCache.del(group.name);
...
SecurityService.prototype.getGroup = function(groupName, options, callback){
    ...
    try{
        ...
        var cachedGroup = __groupCache.get(groupName);

         if (cachedGroup) console.log('cached group:::', cachedGroup);
        ...
        _this.dataService.get(searchPath, {sort:"_id"}, function(e, result){
            ...
            if (result){
                ...
                __groupCache.set(groupName, group);
                callback(null, group);
            }else
                callback(null, null);
        });
...
SecurityService.prototype.getUser = function(userName, options, callback){
    ...
    try{
        ...
        var cachedUser = __userCache.get(userName);
        ...
        _this.dataService.get(searchPath, {sort:"_id"}, function(e, results){
            ...
            if (results.length > 0){
                ...
                __userCache.set(userName, returnUser);
                __passwordCache.set(userName, returnUser.password);
