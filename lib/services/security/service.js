    ...
    _this.dataService.get('/_SYSTEM/_SECURITY/_SETTINGS/KEYPAIR', {}, function(e, response){
        ...
        if (!response){
            ...
            if (typeof config.keyPair != 'string')
                config.keyPair = _this.serializeKeyPair(config.keyPair);
...
SecurityService.prototype.generateKeyPair = function(){
    return crypto.createKeyPair();
}

SecurityService.prototype.encryptAsymmetrical = function(publicKey, privateKey, message){
    return crypto.asymmetricEncrypt(publicKey, privateKey, message);
}

SecurityService.prototype.decryptAsymmetrical = function(publicKey, privateKey, message){
    return crypto.asymmetricDecrypt(publicKey, privateKey, message);
}
...
SecurityService.prototype.generateSession = function(user, callback){
    ...
    if (user.publicKey){
        //we can now teleport a secret using assymetric cryptography
        session.secret = require('shortid').generate();

        session.encryptedSecretPublicKey = this._keyPair.publicKey;
        session.encryptedSecret = this.encryptAsymmetrical(user.publicKey, this._keyPair.privateKey, session.secret);
...
SecurityService.prototype.getPasswordHash = function(username, callback){
    ...
    _this.dataService.get('/_SYSTEM/_SECURITY/_USER/' + username, function(e, user){
        ...
        _this.__passwordCache.set(user.data.username, hash);
    ...
    if (credentials.username && credentials.password) {
        ...
        return _this.getUser(credentials.username, function(e, user){
            ...
            _this.getPasswordHash(credentials.username, function(e, hash){
                 ...
                 _this.verifyHash(credentials.password, hash, function(e, match) {
    ...
    try{
        ...
        _this.dataService.get(searchPath, {sort:"_id"}, function(e, result){
            ...
            if (result){
                ...
                _this.__groupCache.set(groupName, group);//we are passing the back to who knows 
                                                                            //where and it lives here in the cache, not cloing results in it being modified here
