...
SecurityService.prototype.__profileSession = function(session){
  ...
  for (var profileIndex in _this.__cache_Profiles) {
    ...
    console.log('_this.__cache_Profiles 0:::', JSON.stringify(_this.__cache_Profiles[0]));
    console.log('_this.__cache_Profiles 1:::', JSON.stringify(_this.__cache_Profiles[1]));
    console.log('_this.__cache_Profiles session:::', JSON.stringify(session));
  ...
  if (session.profile[0] != null || session.profile[1] != null) return;
...
SecurityService.prototype.generateSession = function (user, callback, type) {
  ...
  if (!type) type = 1;//stateful by default
