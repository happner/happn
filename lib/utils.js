...
module.exports.initialize = function(conf) {
  ...
  if (typeof UTILITIES == 'object' && typeof UTILITIES.createLogger == 'function') {
    // Use existing logger
    module.exports.log = UTILITIES.log;
    module.exports.createLogger = UTILITIES.createLogger;
    return;
  }
  ...
  try {
    ...
    if (logger[guard[level]]()) {
      ...
      if (config.logTimeDelta) {
        message = ((now = Date.now()) - (_this.previous || now)) + 'ms' + delim + '(' + component + ') ' + message;
        _this.previous = now;
      }
      else {
