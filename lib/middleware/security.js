...
SecurityMiddleware.prototype.__respond = function(message, data, error, res, code){
  ...
  if (error) {
    ...
    if (!code) code = 500;
    responseString = responseString.replace("{{ERROR}}", this.happn.utils.stringifyError(error));
  ...
  if (_this.happn.config.secure) {
    ...
    try {
      ...
      if (path == '/auth/request-nonce'){
        ...
        params.publicKey = _this.happn.utils.getFirstMatchingProperty(['publicKey','public_key','public','key','public-key'], query);
      ...
      if (path == '/auth/login'){
        ...
        params.username = _this.happn.utils.getFirstMatchingProperty(['user','username','u'], query);
        params.password = _this.happn.utils.getFirstMatchingProperty(['password','pwd','pass','p'], query);
        params.publicKey = _this.happn.utils.getFirstMatchingProperty(['publicKey','public_key','public','key','public-key','pk'], query);
        params.digest = _this.happn.utils.getFirstMatchingProperty(['digest'], query);
