...
module.exports = {
  ...
  initialize: Promise.promisify(function (callback) {
    try {
      ...
      if (_this.context.config.secure && !_this.options.secure) return callback(_this.securityService.AccessDeniedError('attempt to log on to a secure server'));
  ...
  authorizeRequest: function (message, handler, callback) {
    ...
    var _this = this;
    _this.securityService.authorize(_this.session, message.path, message.action, function (e, authorized, reason) {

      if (e) return _this.pubsub.handleDataResponseLocal(e, message, null, handler, _this);

      if (!authorized) return _this.pubsub.handleDataResponseLocal(_this.securityService.AccessDeniedError('unauthorized', reason), message, null, handler, _this);
  ...
  performRequest: function (path, action, data, options, handler) {
    ...
    _this.setImmediate(function () {
      ...
      _this.validateRequest(path, action, data, options, handler);

      var message = {'path': path, 'action': action, 'eventId': _this.getEventId(), 'options': options};

      if (action == 'login') return _this.__login(message, data, handler);

      if (action == 'describe') {
        var serviceDescription = _this.systemService.getDescription();
        return _this.pubsub.handleDataResponseLocal(null, message, {data: serviceDescription}, handler, _this);
      }
      ...
      _this.authorizeRequest(message, handler, function () {
        ...
        if (action == 'set') {
          ...
          if (options.noStore) return _this.pubsub.handleDataResponseLocal(null, message, _this.dataService.formatSetData(path, data), handler, _this);
