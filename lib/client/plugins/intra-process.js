...
module.exports = {
	...
	authorizeRequest:function(message, parameters, handler, callback){

		if (this.pubsub.trusted[this.session.id] == undefined) return this.pubsub.handleDataResponseLocal(this.securityService.AccessDeniedError(), message, null, handler, this);
		...
		_this.securityService.authorize(_this.session, message.path, message.action, function(e, allowed){
			...
			if (!allowed) return _this.pubsub.handleDataResponseLocal(_this.securityService.AccessDeniedError(), message, null, handler, _this);
	...
	performRequest:function(path, action, data, parameters, handler) {
		...
		_this.authorizeRequest(message, parameters, handler, function(){
	...
	on: Promise.promisify(function(path, parameters, handler, done){
		...
		_this.securityService.authorize(_this.session, path, 'on', function(e, allowed){

			if (!allowed) return done(_this.securityService.AccessDeniedError());
