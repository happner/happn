...
module.exports = {
    ...
    this.context.log.info('XXX - performRequest()');
		...
		this.validateRequest(path, action, data, parameters, handler);

		var message = {"path":path, "action":action, "eventId":this.getEventId(), "parameters":parameters};
        ...
        if (parameters.initialCallback || parameters.initialEmit){
          ...
          _this.dataService.get(path, {sort:{"modified":1}}, function(e, initialItems){
            ...
            if (parameters.initialCallback){
              done(null, listenerId, _this.pubsub.formatReturnItems(initialItems));
            }
            else{
