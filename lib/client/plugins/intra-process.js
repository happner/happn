module.exports = {
	initialize:function(config, done){
		...
		try{
			...
			_this.session = _this.pubsub.connect(_this.handle_publication.bind(_this));
	...
	performRequest:function(path, action, data, parameters, handler){
		...
		if (action == "set"){
			if (parameters.noStore) return _this.pubsub.handleDataResponse(null, message, _this.dataService.formatSetData(path, data), eventSourceProxy);
	...
	_remoteOff:function(channel, decrementBy, done){
		var _this = this;
		try{
			_this.pubsub.removeListener(_this.session.index, channel, decrementBy);
			done();
		}catch(e){
			done(e);
		}
	...
	offAll:function(done){
		...
		try{
			_this.pubsub.removeListener(_this.session.index, '*');
			_this.events = {};
			done();
		}catch(e){
			done(e);
	...
	on:function(path, parameters, handler, done){
		...
		try{
			...
			_this.pubsub.addListener(channel, _this.session.index);
