(function () { // begin enclosed
  ...
  HappnClient.create = Promisify(function (options, callback) {
    ...
    this.__instance(options).initialize(callback);
  });
  ...
  HappnClient.prototype.client = function (options) {
    ...
    if (options.publicKey)
      options.config.publicKey = options.publicKey;

    if (options.privateKey)
      options.config.privateKey = options.privateKey;
    ...
    if (!options.config.pubsub.options.reconnect)
      options.config.pubsub.options.reconnect = {};

    if (options.reconnect)
      options.config.pubsub.options.reconnect = options.reconnect;//override, above config is very convoluted

    if (!options.config.pubsub.options.reconnect.retries)
      options.config.pubsub.options.reconnect.retries = Infinity;

    if (!options.config.pubsub.options.reconnect.max)
      options.config.pubsub.options.reconnect.max = 180000;//3 minutes
  ...
  HappnClient.prototype.loginDigest = Promisify(function (callback) {

    var _this = this;

    if (!_this.options.config.keyPair ||
        !_this.options.config.keyPair.privateKey ||
        !_this.options.config.keyPair.publicKey)
      return callback(new Error('keypair required'));

    _this.__ensureCryptoLibrary(function (e) {

      if (e) return callback(e);

      _this.performRequest(null, 'request-nonce', _this.options.config.keyPair.publicKey, null, function (e, result) {
        ...
        loginParameters.digest = crypto.sign(result, _this.options.config.keyPair.privateKey);
  ...
  HappnClient.prototype.login = Promisify(function (callback) {
    ...
    if (this.options.config.password)
      loginParameters.password = this.options.config.password;

    if (this.options.config.publicKey)
      loginParameters.publicKey = this.options.config.publicKey;
  ...
  HappnClient.prototype.performRequest = function (path, action, data, parameters, callback) {

    if (!this.initialized && ['login', 'describe'].indexOf(action) == -1) return callback('client not initialized yet.');
  ...
  HappnClient.prototype._remoteOn = function (path, parameters, callback) {
    this.performRequest(path, 'on', this.session, parameters, callback);
  }
  ...
  HappnClient.prototype.off = Promisify(function (handle, callback) {
    ...
    if (handle == null || handle == undefined)
      return callback(new Error('handle or callback cannot be null'));

    if (typeof handle == 'function')
      return this.offPath(handle);

    if (typeof handle == 'number')
      return this._offListener(handle, callback);
