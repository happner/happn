...
HappnClient.prototype.login = Promisify(function(done){
	...
	return _this.performRequest(null, 'login', loginParameters, null, function(e, result){
...
HappnClient.prototype.performRequest = function(path, action, data, parameters, done){
	...
	if (parameters.encryptPayload){

		if (!this.session.secret)
			return done(new Error('missing session secret for encrypted payload, did you set the publicKey config option when creating the client?'));

		message.data = crypto.symmetricEncrypt(this.session.secret, null, message.data);
	}
