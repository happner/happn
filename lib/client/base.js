...
HappnClient.prototype.initialize = function(done){
	...
	$.getScript( _this.options.url + '/browser_primus.js', function( data, textStatus, jqxhr ) {
...
HappnClient.prototype.authenticate = function(done){
	...
	if (!_this.pubsub) {
		...
		Socket = Primus.createSocket({ "transformer": _this.options.config.transformer, "parser": _this.options.config.parser, "manual":true });

		_this.pubsub = new Socket(_this.options.config.url);
	...
	/*BEGIN CLIENT-SIDE ADAPTER - DO NOT REMOVE THIS COMMENT
		if (!_this.pubsub) {
			_this.pubsub = Primus.connect(_this.options.config.url, _this.options.config.pubsub.options);
			_this.pubsub.on('error',  _this.handle_error.bind(_this));
			_this.pubsub.on('data', _this.handle_publication.bind(_this));
			_this.pubsub.on('reconnected', _this.handle_reconnection.bind(_this));
		}
	*///END CLIENT-SIDE ADAPTER
	...
	return _this.performRequest(null, 'login', {secret:_this.options.config.secret}, null, function(e, result){
