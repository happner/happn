function HappnClient(options, initializedCallback){
	...
	if (!options.config)
		options.config = {host:'127.0.0.1', port:8000, secret:'happn'};
	...
	_this.handle_reconnection = function(options){
		...
		_this.authenticate(function(e){
			...
			async.eachSeries(Object.keys(_this.events), 
			function(eventPath, eachCallback){
				var listener = _this.events[eventPath];
				_this._remoteOn(eventPath, listener.id, eachCallback);
				attachedCount++;
			}, 
			function(e){
				if (e) return _this.handle_error(e, 3);
				////console.log('reconnected ok, ' + attachedCount + ' events were re-attached');
			});
		});
	}
	...
	_this.handle_error = function(err, severity){
		...
		console.error('Error happened over transport: ' + err.toString(), err.stack);
	};
	...
	_this._remoteOn = function(path, listenerId, done){
		var _this = this;
		_this.performRequest(path, 'on', _this.session, {"listenerId":listenerId}, done);
	}
	...
	_this._remoteOff = function(channel, decrementBy, done){
		...
		_this.performRequest(channel, 'off', _this.session, {"decrementBy":decrementBy}, function(e, response){
			if (e)
				return done(e);
