(function () { // begin enclosed
  ...
  HappnClient.prototype.login = Promisify(function (callback) {
    ...
    var loginParameters = {
      username: _this.options.config.username,
    ...
    if (this.options.config.token) loginParameters.token = this.options.config.token;
  ...
  HappnClient.prototype.performRequest = function (path, action, data, options, callback) {
    ...
    if (!options.timeout) options.timeout = 60000;//this is not used on the server side
    ...
    _this._remoteOn(path, parameters, function (e, response) {
      ...
      if (e) {
        ...
        for (var listenerIndex in _this.events[path]) {
          ...
          if (listenerToRemove.id == parameters.refCount) {
  ...
  HappnClient.prototype.__revokeSession = function(callback){

    this.performRequest(null, 'revoke-session', null, null, callback);
  };

  HappnClient.prototype.disconnect = Promisify(function (opts, callback) {
    ...
    if (this.initialized) {
      ...
      return _this.offAll(function (e) {
        ...
        if (opts && opts.revokeSession){

          _this.__revokeSession(function(e){

            if (e) return callback(new Error('error revoking session: ' + e.toString()));

            destroyPubSub(_this, callback);
          });

        } else destroyPubSub(_this, callback);
      });
    }
    else {
      setTimeout(function () {
