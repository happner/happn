...
HappnClient.prototype.authenticate = function(done){
	...
	if (browser) {
		if (!this.pubsub) {
			...
			this.pubsub.on('connected', function() {delete connecting});
	...
	return this.performRequest(null, 'login', {
		...
		if (typeof connecting != 'undefined' && connecting == false) {
			...
			// Made connection attempt and failed, done() already called
			// (very strange if this occurs)
			return;
		}
...
HappnClient.prototype.handle_error = function(err, severity){
	...
	if (this.errors.length >= 100)
		...
		this.log.error('Socket TT error', err);
...
HappnClient.prototype.handle_publication = function(message){
	...
	if (message._meta.type == 'data'){
		// receive published
	  this.handle_data(message._meta.channel, message);
	  return;
  }
	...
	if(!message.__NULL) {
		if (message.paths) {
			...
			// 	configurable: true
			// });
			message.__ARRAY._meta = message._meta;
  ...
  if (decoded && decoded._meta.type == 'message'){
  	this.handle_message(decoded);
  }
  else if (decoded && decoded._meta.type == "response"){
