(function () { // begin enclosed
  ...
  HappnClient.prototype._remoteOn = function (path, parameters, done) {
    this.performRequest(path, 'on', this.session, parameters, done);
  }
  ...
  HappnClient.prototype.on = Promisify(function (path, parameters, handler, done) {
    ...
    _this._remoteOn(path, parameters, function (e, response) {
      ...
      done(null, parameters.refCount, response);
    });
  });
  ...
  HappnClient.prototype._offListener = function (listenerId, done) {
    ...
    if (!_this.events || _this.events.length == 0) return done();
  ...
  HappnClient.prototype.off = Promisify(function (handle, done) {

    if (handle == null || handle == undefined)
      return done(new Error('handle or callback cannot be null'));

    if (typeof handle == 'function')
      return this.offPath(handle);

    if (typeof handle == "number")
      return this._offListener(handle, done);

    console.warn('.off with a path is deprecated, please use the offPath method for a path based unsubscribe');

    return this._offPath(handle, done);
  });

  HappnClient.prototype.offPath = Promisify(function (path, done) {
    ...
    return this._offPath(path, done);
  });
