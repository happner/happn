...
if (!browser) {
	...
	Crypto = require('consent-util-crypto');
...
HappnClient.prototype.__encryptPayload = function(message){
	...
	console.log('encrypting payload:::', message);

	return {
		sessionId:message.sessionId,
		encrypted:crypto.symmetricEncrypt(this.session.secret, this.session.secretSalt, JSON.stringify(message))
	}
}

HappnClient.prototype.__decryptPayload = function(message){

	var decrypted = JSON.parse(crypto.symmetricDecrypt(this.session.secret, this.session.secretSalt, message));
...
HappnClient.prototype.login = Promisify(function(done){
	...
	_this.performRequest(null, 'describe', null, null, function(e, serverInfo){
		...
		console.log('server info:::', _this.serverInfo);
		...
		_this.performRequest(null, 'login', loginParameters, null, function(e, result){
			...
			if (result._meta.status == 'ok') {
				...
				if (browser)
					document.cookie = 'happn_token=' + _this.session.id;

				if (result.encryptedSecret){
					_this.session.secret = crypto.asymmetricDecrypt(result.encryptedSecretPublicKey, _this.options.config.keyPair.privateKey, result.encryptedSecret).toString();
				}
...
HappnClient.prototype.handle_publication = function(message){
	...
	console.log('handle_publication:::', message);
...
HappnClient.prototype.handle_response = function(e, response){
	...
	console.log('have handler:::', responseHandler, response._meta.eventId, this.requestEvents);
