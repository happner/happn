...
HappnClient.prototype.handle_publication = function(message){
	...
	if (message._.type == 'data'){
	  	this.handle_data(message._.channel, message);
  	}else if (message._.type == 'message'){
  		this.handle_message(message);
  	}else if (message._.type == "response"){
  		this.handle_response(message);
  	}
};

HappnClient.prototype.handle_response = function(response){
	var responseHandler = this.requestEvents[response._.eventId];
