...
HappnClient.prototype.client = Promise.promisify(function(options, done){
	var _this = this;
...
// HappnClient.prototype.getChild = Promise.promisify(function(path, childId, handler){
// 	this.get(path, {options:{child_id:childId}}, handler);
// });
...
HappnClient.prototype.decodeArrayResponse = function(message) {
	...
	decoded._event = message._event;
	return decoded;
}

HappnClient.prototype.handle_publication = function(message){

	console.log('TODO, channel, message')

	var decoded;

	if (message.paths) {
		decoded = message.paths;
		decoded._event = message._event;
	}
	else if (message.__ARRAY) {
		message.__ARRAY._event = message._event;
		decoded = this.decodeArrayResponse(message.__ARRAY);
	} else {
		decoded = message.data;
		decoded._store = message._store;
		decoded._event = message._event;
	...
	if (decoded._event.type == 'data'){
	  	this.handle_data(decoded._event.channel, decoded);
  	}else if (decoded._event.type == 'message'){
  		this.handle_message(decoded);
  	}else if (decoded._event.type == "response"){
  		this.handle_response(decoded);
  	}
};

HappnClient.prototype.handle_response = function(response){
	var responseHandler = this.requestEvents[response._event.id];
