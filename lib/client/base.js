(function () { // begin enclosed
  ...
  HappnClient.prototype.__prepareLogin = function(loginParameters, callback){
    ...
    if (loginParameters.loginType == 'digest'){
      ...
      //path, action, data, options, callback

      _this.performRequest(null, 'request-nonce', {publicKey:loginParameters.publicKey}, null, function (e, nonce) {

        if (e) return callback(e);
        loginParameters.digest = crypto.sign(nonce, _this.options.config.keyPair.privateKey);
  ...
  HappnClient.prototype.login = Promisify(function (callback) {
    ...
    _this.performRequest(null, 'describe', null, null, function (e, serverInfo) {
      ...
      if (_this.serverInfo.encryptPayloads || loginParameters.loginType === 'digest') {
        ...
        _this.__ensureCryptoLibrary(function (e) {
          ...
          if (!_this.options.config.keyPair) {
            ...
            if (loginParameters.loginType === 'digest') return callback(new Error('login type is digest, but no keypair specified'));
      ...
      } else doLogin(loginParameters, callback);
  ...
  HappnClient.prototype.performRequest = function (path, action, data, options, callback) {
    ...
    var message = {'path': path, 'action': action, 'eventId': eventId, 'options': options, 'data': data};
