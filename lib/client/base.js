...
HappnClient.prototype.login = Promisify(function(done){
	...
	return _this.performRequest(null, 'login', {
		...
		publicKey: this.options.config.publicKey,
		...
		if (result._meta.status == 'ok') {
			...
			if (result.encryptedSecret){
				_this.session.secret = crypto.asymmetricDecrypt(result.encryptedSecretPublicKey, _this.options.config.privateKey, result.encryptedSecret).toString();
			}
