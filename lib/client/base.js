...
HappnClient.prototype.login = Promise.promisify(function(done){
	...
	return _this.performRequest(null, 'login', {
		...
		if (typeof _this.__connecting != 'undefined' && _this.__connecting == false) {

			// Made connection attempt and failed, done() already called
			// (very strange if this occurs)
			// TODO - umm... why does this ever happn?
			return;
		}
...
HappnClient.prototype.authenticate = Promise.promisify(function(done){
	...
	if (browser) {
		if (!this.pubsub) {
			...
			this.pubsub.on('connected', function() {
...
HappnClient.prototype.offSystemMessage = function(key, handler){
	
};

HappnClient.prototype.onSystemMessage = function(key, handler){
