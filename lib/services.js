...
module.exports = {
    ...
    var wildcardCache = config.services.wildcardCache = config.services.wildcardCache || {};
    wildcardCache.path = wildcardCache.path || './services/wildcard-cache/service.js';
    wildcardCache.config = wildcardCache.config || {};
    wildcardCache.config.max = wildcardCache.config.max || 0;       // default disabled (no cache)
    wildcardCache.config.maxAge = wildcardCache.config.maxAge || 0; // default no entries in the cache expire
      	...
      	async.eachSeries(['crypto','data', 'system', 'security', 'pubsub', 'wildcardCache'],
